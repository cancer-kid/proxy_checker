...
def valid_lat(response_elapsed, max_latency=10):
    time = str(response_elapsed).split(":")
    if int(time[2].split(".")[0]) <= int(max_latency) and time[0] == "0" and time[1] == "00":
        return True
    else: return False
...
SetupThreads()
print("Checking proxies with 10sec max latency...")
print("This might take a while.")
StartThreads()
AwaitActiveThreads()
print("Done!")
#result
print("{} out of 1588 proxys are active.\n".format(str(len(active_proxies))))
print("Ipv4 : Port   |   Latency")
for entry in active_proxies:
    if valid_lat(entry[-1]):
